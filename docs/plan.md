...
**Foundation toolchain implemented under #1; no enforcement runtime.** Product direction, security contracts, and the backlog remain foundation-only. The TypeScript source still contains no policy engine, vault, egress sentinel, or privacy boundary; milestone 0's contract/security tests depend on later issues.
